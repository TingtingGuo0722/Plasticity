...
setwd("D:/Sorghum_GxE_Project/Wheat_Data_from_Xianran/Rice/Ceres")
source('sub_funcs_20180329.r');
######################################
col_wdw <- 25;
col_palette <- diverge_hcl(col_wdw + 1, h = c(260, 0), c = 100, l = c(50, 90), power = 1)

Dir <- 'D:/Sorghum_GxE_Project/';
sp <- 'Wheat_Data_from_Xianran';
exps <- 'Rice/Ceres';
...
env_meta_file <- paste(exp_dir, '9Env_meta_table', sep = ''); ## S2_Met_22Env_meta_table
env_meta_info_0 <- read.table(env_meta_file, header = T, sep = "\t", stringsAsFactors = F);

exp_s_year <- min(env_meta_info_0$TrialYear); #exp_s_day <- paste(exp_s_year, '0101', sep = "\t"); 
exp_e_year <- max(env_meta_info_0$TrialYear) + 1; #exp_e_day <- paste(exp_e_year, '1231', sep = "\t");

searching_daps <- 200;
ptt_ptr_file <- paste(exp_dir, nrow(env_meta_info_0), 'Envs_envParas_DAP',  searching_daps, sep = ''); ##  '_adjGDD',
if (!file.exists(ptt_ptr_file)) { Compile_PTT_PTR(exp_dir, env_meta_info_0, exp_s_year, exp_e_year, searching_daps, ptt_ptr_file, t_base, t_max1, t_max2) };

PTT_PTR <- read.table(ptt_ptr_file, header = T , sep = "\t");


exp_traits_file <- paste(exp_dir, 'Trait_Rice', sep = '');
...
lInd <- which(colnames(exp_traits) == 'line_code'); eInd <- which(colnames(exp_traits) == 'env_code'); 
...
## FTdap
#env_outliers <- c('CS11_Lacombe'); ##c('UEOPN_2000_Brookings', 'UMOPN_2007_Morris', 'UMOPN_2005_Watertown', 'UMOPN_2007_Watertown'); ##, 'HNY15', 'KNY15', 'BZD15', 'CRM15', , 'CHM16'
## PH
#env_outliers <- c('CS11_Ithaca'); ##c('UEOPN_2000_Brookings', 'UMOPN_2007_Morris', 'UMOPN_2005_Watertown', 'UMOPN_2007_Watertown'); ##, 'HNY15', 'KNY15', 'BZD15', 'CRM15', , 'CHM16'
## GY
...
#searching_daps <- 150;
Exhaustive_search(env_mean_trait, PTT_PTR, searching_daps, exp_trait_dir, exp_traits$FTdap, trait, 1, searching_daps, searching_daps, LOO=1)#; searching_daps, searching_daps);
...
#### LOOCV function
obs_prd_file <- paste(exp_trait_dir, trait, '_', nrow(env_mean_trait), 'Env_LOO_by_Lines_', kPara_Name, '_D', maxR_dap1, '_', maxR_dap2, sep = '');
LOO_pdf_file <- paste(exp_trait_dir, trait, '_', nrow(env_mean_trait), 'Env_LOO_by_Lines_', kPara_Name, '_D', maxR_dap1, '_', maxR_dap2, '.pdf', sep = '');
if (!file.exists(obs_prd_file)) { 
  prdM <- LOOCV(maxR_dap1, maxR_dap2, env_mean_trait, PTT_PTR, PTT_PTR_ind, exp_trait,  obs_prd_file)
}
Plot_prediction_result(obs_prd_file, all_env_code, prdM, LOO_pdf_file);


# #### Forecasting next year
# trn_year <- 2010;
# env_outliers <- c('CS11_Lacombe');
# 
# exp_trait_forecast <- exp_traits[,c(lInd, eInd, tInd)];
# colnames(exp_trait_forecast)[3] <- 'Yobs';
# exp_trait_forecast <- exp_trait_forecast[!is.na(exp_trait_forecast$Yobs),];
# exp_trait_forecast <- exp_trait_forecast[!(exp_trait_forecast$env_code %in% env_outliers),];
# 
# trn_env <- env_meta_info_0$env_code[env_meta_info_0$TrialYear == trn_year];
# exp_trait_trn <- exp_trait[(exp_trait$env_code %in% trn_env),];
# line_codes_trn <- unique(exp_trait_trn$line_code);
# env_mean_trait_trn_0 <- na.omit(aggregate(x = exp_trait_trn$Yobs, by = list(env_code = exp_trait_trn$env_code), mean, na.rm = T));
# colnames(env_mean_trait_trn_0)[2] <- 'meanY';
# env_mean_trait_trn <- env_mean_trait_trn_0[order(env_mean_trait_trn_0$meanY),];
# exp_trait_dir <- paste(exp_dir, trait, '/Forecast/', sep = ''); if (!dir.exists(exp_trait_dir))  { dir.create(exp_trait_dir)};
# Exhaustive_search(env_mean_trait_trn, PTT_PTR, searching_daps, exp_trait_dir, exp_traits$FTdap, trait, 1, searching_daps, searching_daps)#; searching_daps, searching_daps);
# 
# p <- 1;
# maxR_dap1 <- 72;
# maxR_dap2 <- 79;
# kPara_Name <- 'PTR';
# PTT_PTR_ind <-  which(colnames(PTT_PTR) == kPara); ## DL -> 5, GDD -> 6; PTT -> 7; PTR -> 8; PTD1 -> 8, PTD2 -> 9, PTS -> 10
# 
# obs_prd_file <- paste(exp_trait_dir, trait, '_', nrow(env_mean_trait_trn), 'Forecast', kPara_Name, '_D', maxR_dap1, '_', maxR_dap2, sep = '');
# forecast_pdf_file <- paste(exp_trait_dir, trait, '_', nrow(env_mean_trait_trn), 'Forecast_', kPara_Name, '_D', maxR_dap1, '_', maxR_dap2, '.pdf', sep = '');
# if (!file.exists(obs_prd_file)) { 
#   prdM <- Forecast_next_year(maxR_dap1, maxR_dap2, env_mean_trait, PTT_PTR, PTT_PTR_ind, exp_trait, trn_env, obs_prd_file)
# }
# 
# Plot_prediction_result(obs_prd_file, all_env_code, prdM, forecast_pdf_file)
